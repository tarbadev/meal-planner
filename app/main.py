...
def _run_migrations() -> None:
    ...
    Called in a thread pool from the async lifespan so the event loop is not
    blocked.  Each migration runs inside a transaction; a failure rolls back
    automatically and re-raises, which prevents the app from starting.
    """
...
async def lifespan(app: FastAPI):
    """Startup: run DB migrations, validate config, seed default household.
    Shutdown: dispose the async engine.
    """
    # Run migrations before accepting traffic; failure aborts startup.
