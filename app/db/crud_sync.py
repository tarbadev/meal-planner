...
def get_recipes(
    ...
    stmt = select(RecipeModel).where(RecipeModel.household_id == household_id)
...
def get_recipe_by_id(db: Session, recipe_id: str) -> Recipe | None:
    ...
    result = db.execute(select(RecipeModel).where(RecipeModel.id == recipe_id))
...
def upsert_recipe(db: Session, recipe: Recipe, household_id: str) -> Recipe:
    ...
    existing = db.execute(
        select(RecipeModel).where(RecipeModel.id == recipe.id)
    ).scalar_one_or_none()
